...
(function () {
    ...
    function tileHTML(b, inLib) {
        ...
        var parts = [];
        if (typeof b.avg !== "undefined" && b.avg !== null) parts.push("★ " + Number(b.avg).toFixed(1));
        if (b.year) parts.push(String(b.year));
        var metaHTML = '<div class="meta">' + parts.join(" · ") + '</div>';
        var chips = '<div class="chips">' + genres.map(function (g) { return '<span class="chip muted-small">' + esc(cap(g)) + '</span>'; }).join("") + '</div>';
        ...
        sec.addEventListener("click", async function (e) {
            ...
            if (add) {
                ...
                try {
                    ...
                    console.warn(err); add.disabled = false; alert("Could not add this book.");
                }
                return;
            }
