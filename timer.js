...
(function () {
  ...
  const loadState = () => {
    try {
      ...
      console.warn("Timer state was corrupt, resetting.", e);
  ...
  function normalizeState(s) {
    try {
      ...
      if (!s.startAt && !s.accumMs) {
        clearState();
        return null;
      }
      if (s.pausedAt) {
        const pausedDate = new Date(s.pausedAt);
        if (isNaN(pausedDate.getTime()) || toDayStr(pausedDate) !== toDayStr(new Date())) {
          clearState(); drawTime(0); return null;
        }
      }
    } catch (e) {
      console.error("Timer state normalization failed, resetting.", e);
